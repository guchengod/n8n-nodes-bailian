import { INodeProperties } from 'n8n-workflow';

export const TaskResultOperations: INodeProperties[] = [
	{
		displayName: '操作',
		name: 'operation',
		type: 'options',
		noDataExpression: true,
		displayOptions: {
			show: {
				resource: ['taskResult'],
			},
		},
		options: [
			{
				name: '查询图像任务结果',
				value: 'queryImageTask',
				description: '查询图像生成、编辑任务的异步结果',
				action: '查询图像任务',
			},
			{
				name: '查询视频任务结果',
				value: 'queryVideoTask',
				description: '查询视频生成、编辑、数字人任务的异步结果',
				action: '查询视频任务',
			},
			{
				name: '查询语音任务结果',
				value: 'querySpeechTask',
				description: '查询录音文件转写、音视频翻译任务的异步结果',
				action: '查询语音任务',
			},
			{
				name: '查询向量任务结果',
				value: 'queryVectorTask',
				description: '查询批处理向量任务的异步结果',
				action: '查询向量任务',
			},
		],
		default: 'queryImageTask',
	},
];

const commonTaskFields: INodeProperties[] = [
	{
		displayName: '任务 ID',
		name: 'taskId',
		type: 'string',
		default: '',
		placeholder: '例如：6552xxxxxxxxxxxxx',
		description: '需要查询的异步任务 ID',
		required: true,
	},
	{
		displayName: '是否持续轮询直至完成',
		name: 'waitingForTask',
		type: 'boolean',
		default: true,
		description: 'Whether to wait and poll until the task succeeds or fails (是否等待并轮询直到任务成功或失败)',
	},
	{
		displayName: '轮询间隔 (毫秒)',
		name: 'interval',
		type: 'number',
		displayOptions: {
			show: {
				waitingForTask: [true],
			},
		},
		default: 3000,
	},
	{
		displayName: '最长等待时间 (秒)',
		name: 'maxWaitTime',
		type: 'number',
		displayOptions: {
			show: {
				waitingForTask: [true],
			},
		},
		default: 600,
		description: '等待任务完成的最长时间',
	},
	{
		displayName: '如果任务失败怎么处理',
		name: 'continueOnFail',
		type: 'boolean',
		default: true,
		description: 'Whether to continue workflow execution even if the task fails (即使任务失败也继续工作流执行)',
	},
];

export const TaskResultFields: INodeProperties[] = [
	...commonTaskFields.map(field => ({
		...field,
		displayOptions: {
			show: {
				resource: ['taskResult'],
				operation: ['queryImageTask', 'queryVideoTask', 'querySpeechTask', 'queryVectorTask'],
			},
		},
	})),
];
